...
class products{
    ...
    public function select() {
		...
		$query_ads = mysqli_query("SELECT * , 111.045 * DEGREES( ACOS( COS( RADIANS( ".$lan." ) ) * COS( RADIANS(  `pic_add_lan` ) ) * COS( RADIANS(  `pic_add_lon` ) - RADIANS( ".$lon." ) ) + SIN( RADIANS( ".$lan." ) ) * SIN( RADIANS(  `pic_add_lan` ) ) ) ) AS distance FROM pic_addpost where pic_category in ".$cat_char." and pic_request=".$_REQUEST['type']." and addpost_status=1 order by ".$order." LIMIT 5 OFFSET ".$_REQUEST['offset']."");
		?>
		...
		?>
        	...
        	<div class="list-view">
                ...
                <?php
				$query_img = mysqli_query("select * from pic_addpost_images where addpost_id='$row->pic_ads_id' order by ad_image_id ASC limit 1");
    ...
    <h4>
        <?php
        $title_query = mysqli_query("SELECT * FROM `pic_categories_fields`  where `fields_id`=".$_REQUEST['sub']." limit 1");
	...
	<select class="filter_chain filter_sub" name="<?php echo $_REQUEST['sub'];?>" id="<?php echo $_REQUEST['sub']; ?>" onchange="javascript:filterProducts(this,<?php echo $_REQUEST['cat_id']; ?>,<?php echo $_REQUEST['type']; ?>,<?php echo $_REQUEST['p']; ?>,<?php echo $_REQUEST['sort']; ?>);"  style="width:78%;">
		...
		$sub_quick_filter_query = mysqli_query("SELECT * FROM `pic_categories_fields`  where `field_chain_value` LIKE '%{$strr}%' ");
		?>
	 ...
	 public function list_products() {
            ...
            $cat_ids= $_GET['cat_id'];
			$subcat_query = mysqli_query("SELECT * FROM  `pic_categories` where categories_id=$cat_ids");
            ...
            $user_types=mysqli_query("select * from pic_user where user_id='$usr' ");
            $row = mysqli_fetch_object($user_types);
            $utype = $row->user_type;
            
            $user_types_query=mysqli_query("SELECT * FROM  `pic_user_type` where user_type='$utype'"); 
...
$check_privacy = mysqli_query("SELECT * , 111.045 * DEGREES( ACOS( COS( RADIANS( ".$lan." ) ) * COS( RADIANS(  `pic_add_lan` ) ) * COS( RADIANS(  `pic_add_lon` ) - RADIANS( ".$lon." ) ) + SIN( RADIANS( ".$lan." ) ) * SIN( RADIANS(  `pic_add_lan` ) ) ) ) AS distance FROM pic_addpost where pic_category in ".$category_privacy." and pic_request=".$_REQUEST['type']." and addpost_status=1 order by ".$order."  LIMIT 5 OFFSET ".$_REQUEST['offset']."");
...
//echo "SELECT * , ( 3956 *2 * ASIN( SQRT( POWER( SIN( ( 10.6307 - pic_add_lan ) * PI( ) /180 /2 ) , 2 ) + COS( 10.6307 * PI( ) /180 ) * COS( pic_add_lan * PI( ) /180 ) * POWER( SIN( ( 79.3131 - pic_add_lon ) * PI( ) /180 /2 ) , 2 ) ) ) ) AS distance FROM pic_addpost HAVING distance <=20 and pic_category in ".$category_privacy." and pic_request=".$_REQUEST['type']." and addpost_status=1 order by distance LIMIT 5 OFFSET ".$_REQUEST['offset']."";
			...
			//$check_privacy = mysqli_query("select * from pic_addpost where pic_category in ".$category_privacy." and pic_request=".$_REQUEST['type']." and addpost_status=1 order by $order LIMIT 5 OFFSET ".$_REQUEST['offset']."");
          ...
          if (restore) selObj.selectedIndex=0;
			...
			$query_ads_for_count = mysqli_query("select * from pic_addpost where pic_category in ".$cat_char." and pic_request=".$_REQUEST['type']." and addpost_status=1 order by $order");
...
// main query
$price_filter_query=mysqli_query("select DISTINCT addpost_fields_title from pic_addpost_field where addpost_fields_categories_id in ".$cat_char." ORDER BY addpost_field_id ASC ");
?>
...
<?php
$filter_list_ASC_query=mysqli_query("select * from pic_addpost where pic_category in ".$cat_char." ORDER BY pic_price ASC limit 1");
$filter_list_DESC_query=mysqli_query("select * from pic_addpost where pic_category in ".$cat_char." ORDER BY pic_price DESC limit 1");
...
<?php
$quick_filter_query = mysqli_query("SELECT * FROM `pic_categories_fields` where `fields_type`='Chain' and fields_categories_id=".$_REQUEST['cat_id']."");
...
<?php
		...
		$title_query1 = mysqli_query("SELECT * FROM `pic_categories_fields`  where `fields_id`=$chain_id2 limit 1");
		$title_row1 = mysqli_fetch_object($title_query1);
		
		$chain2 = $title_row1->fields_title;
		
		$title_query = mysqli_query("SELECT * FROM `pic_categories_fields`  where `fields_id`=$chain_id1 limit 1");
		$title_row = mysqli_fetch_object($title_query);
    ...
    <li>
		...
		<?php
        $parent_quick_filter_query = mysqli_query("SELECT * FROM `pic_categories_fields`  where `field_DV_id` in $field_value_trim");
		?>
...
<li>
   		...
   		<?php
			$filter_query=mysqli_query("select DISTINCT field_id,addpost_fields_title,addpost_fields_type from pic_addpost_field where addpost_fields_categories_id in ".$cat_char." and addpost_fields_type in ('TextBox','Numeric')");
			$no=0;
	...
	<a style="background:#6a2800;color:#fff;" href="javascript:void(0);"><?php echo $row->addpost_fields_title; ?></a>
		...
		<?php
			...
			$filter_list_query=mysqli_query("select DISTINCT addpost_fields_value,addpost_fields_title,field_id from pic_addpost_field where addpost_fields_categories_id in ".$cat_char." and field_id = '".$row->field_id."' and addpost_fields_value != '' order by addpost_fields_value ASC");
			$no = mysqli_num_rows($filter_list_query);
			while($row_list = mysqli_fetch_object($filter_list_query)){
			$query_othercolumn=mysqli_query("SELECT * FROM  `pic_categories_fields` where fields_id = '".$row_list->field_id."'");
...
<?php
			$filter_query=mysqli_query("select * from pic_addpost_field where addpost_fields_categories_id in ".$cat_char." and addpost_fields_type='DropDown' and addpost_fields_title != '".$chain1."' and addpost_fields_title != '".$chain2."' group by addpost_fields_title");
			$no=0;
			while($row = mysqli_fetch_object($filter_query)){
        ...
        <li>
        <?php
	$query_title = mysqli_query("SELECT * FROM  `pic_addpost_field` WHERE  `field_id` = $row->field_id limit 1");
	...
	<a style="background:#6a2800;color:#fff;" href="javascript:void(0);"><?php echo $field_title->addpost_fields_title; ?></a>
		<ul <?php if(isset($_POST[$nametitle])){ ?> style="display:block;" <?php } else { ?>style="display:none;" <?php } ?>>
			...
			<div style="overflow: auto; overflow-x: hidden;" id="<?php echo $nametitle ; ?>" class="list-group">
		<?php
			$filter_list_query=mysqli_query("select DISTINCT addpost_fields_value from pic_addpost_field where field_id = $row->field_id and addpost_fields_value!='' order by addpost_fields_value ASC");
			$no = mysqli_num_rows($filter_list_query);
			while($row_list = mysqli_fetch_object($filter_list_query)){
			
			$query_othercolumn=mysqli_query("SELECT * FROM  `pic_categories_fields` where fields_id = '".$row_list->addpost_fields_value."'");
			$row_othercolumn = mysqli_fetch_object($query_othercolumn);
  ...
  <div align="center"><a href="javascript:void(0);">Show Less</a>
      ...
      <div id="loading_filter" style="display:none;">
          ...
          if (restore) selObj.selectedIndex=0;
			...
			if($countAll!=1){
			$query_ads = mysqli_query("SELECT addpost_fields_value,addpost_fields_categories_id,addpost_uni_id,".$str_distance." FROM pic_addpost_field where ".$fvstr.$or.$str." GROUP BY addpost_uni_id having count(distinct addpost_fields_value) = ".$countAll." order by distance ASC");
			
		    $count_rows = mysqli_num_rows($query_ads);
			$count_query = mysqli_query("SELECT addpost_fields_value,addpost_fields_categories_id,addpost_uni_id,".$str_distance." FROM pic_addpost_field where ".$fvstr.$or.$str." GROUP BY addpost_uni_id having count(distinct addpost_fields_value) = ".$countAll." LIMIT 1");
			}
			else{
			$query_ads = mysqli_query("SELECT addpost_fields_value,addpost_fields_categories_id,addpost_uni_id,".$str_distance." FROM pic_addpost_field where ".$fvstr.$or.$str." GROUP BY addpost_uni_id order by distance ASC");
			
            $count_rows = mysqli_num_rows($query_ads);
			$count_query = mysqli_query("SELECT addpost_fields_value,addpost_fields_categories_id,addpost_uni_id,".$str_distance." FROM pic_addpost_field where ".$fvstr.$or.$str." GROUP BY addpost_uni_id  LIMIT 1");
			}
			...
			$query_row = mysqli_query("select * from pic_addpost where ".$priceString." pic_ads_id='".$list->addpost_uni_id."' and pic_request=".$_REQUEST['type']." and addpost_status=1");
			...
			$query_row = mysqli_query("select * from pic_addpost where pic_category in ".$cat_char." and ".$priceString." pic_request=".$_REQUEST['type']." and addpost_status=1 and pic_add_lan>");
	 ...
	 <script>
	  ...
	  public function subcatid(){
		$query_subcat = mysqli_query("SELECT * FROM  `pic_categories` where categories_sub = ".$_REQUEST['cat_id']."");
		$cat_char="(";
