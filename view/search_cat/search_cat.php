...
class search_cat extends config
{
...
?>
		...
		<script>
			...
			function loadmore_search(page, type, cat_id, sort,c) {
...
AND pic_categories.categories_id = pic_addpost.pic_category )  WHERE  (addpost_status=1) and (`pic_title` LIKE  '%$query_search%' or `pic_tag` LIKE  '%$query_search%' or `pic_ads_id` LIKE  '%$query_search%' or `pic_admin_tag` LIKE  '%$query_search%' or `ad_image_desc` LIKE  '%$query_search%') group by pic_category limit 10");
				...
				<div class="row no-gutters">
					...
					<div class="col-md-8">
						<div class="card-body text-left">
							...
							?>
									<p class="hint"><i class="fa fa-map-marker"></i> <?php echo $locRow->loc_name; ?></p>
								<?php }
							} else { ?>
								<p class="hint"><i class="fa fa-map-marker"></i> <?php echo $pic_add_taluk . " , " . $pic_post_city . " District"; ?></p>
							<?php } ?>
							<p class="card-text">
					...
					<div class="col-sm-12 col-md-12 col-lg-8 pb-0">
						...
						<script>
							function MM_jumpMenu(targ, selObj, restore) { //v3.0
								...
								eval(targ + ".location='" + "?module=search&action=view&p=1&post=form&search_pic=<?php echo $_REQUEST['search_pic']; ?>&offset=0&type=" + $("#adstype").val() + "&sort=" + $("#sorting").val() + "'");
						...
						<div class="row" id="ads_div">
							<?php
							$query_search = $_REQUEST['search_pic'];
						...
						if ($count_rows > 4) { ?>
							...
							<div class="d-flex justify-content-center row"><a class="btn btn-primary btn-lg loadmore-products" href="javascript:void(0)" onclick="javascript:loadmore_mobile_layout();">Load More Ads 1</a></div>
						<?php } ?>
	...
	public function list_products_loadmore_cat()
	{
		$this->headerscript();
					...
					<div class="card">
						...
						<div id="collapseTwo" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordion">
							<div class="card-body">
								<?php
								$dates = date("Y-m-d");
								$filter_query = mysqli_query($this->mysqlConfig(), "select DISTINCT pic_category from pic_addpost where addpost_status = 1 and (pic_validity > $dates or pic_validity_auto=1) and pic_request=" . $_REQUEST['type'] . $filter_str . " and (`pic_title` LIKE  '%$query_search%' or `pic_tag` LIKE  '%$query_search%' or `pic_ads_id` LIKE  '%$query_search%' or `pic_admin_tag` LIKE  '%$query_search%') order by $order");
									...
									<ul class="list-group">
										...
										?>
												<li class="list-group-item">
													<input <?php if (isset($_POST['cat_id']) and $_POST['cat_id'] == $cat_id_selected) { ?> checked <?php } else {
																																			} ?> id="category" name="category" value="<?php echo $cat_id_selected; ?>" type="radio" onchange="javascript:filterProducts_search(this,'<?php echo $cat_id_selected; ?>',<?php echo $_REQUEST['type']; ?>,<?php echo $_REQUEST['p']; ?>,<?php echo $_REQUEST['sort']; ?>,'<?php echo $query_search; ?>','<?php if (isset($_POST['location']) and $_POST['location'] != "") {
																																																																																																																			echo $_POST['location'];
																																																																																																																		} else { ?><?php } ?>','<?php if (isset($_POST['nearer']) and $_POST['nearer'] != "") {
																																																																																																																																																echo $_POST['nearer'];
																																																																																																																																															} else { ?><?php } ?>');" class="form-check-input filter_checkbox m-1">
						...
						<div id="collapseThree" class="collapse show" aria-labelledby="headingThree" data-parent="#accordion">
							<div class="card-body">
								<?php
								$filter_query = mysqli_query($this->mysqlConfig(), "select DISTINCT pic_post_city from pic_addpost where  addpost_status = 1 and (pic_validity > $dates or pic_validity_auto=1) and pic_request=" . $_REQUEST['type'] . $filter_str . " and (`pic_title` LIKE  '%$query_search%' or `pic_tag` LIKE  '%$query_search%' or `pic_ads_id` LIKE  '%$query_search%' or `pic_admin_tag` LIKE  '%$query_search%') order by $order");
								...
								?>
									...
									<ul class="list-group">
										...
										?>
												<li class="list-group-item">
													<input <?php if (isset($_POST['location']) and $_POST['location'] == $name) { ?> checked <?php } else {
																																		} ?> id="location" name="location" value="<?php echo $name; ?>" type="radio" onchange="javascript:filterProducts_search(this,'<?php if (isset($_POST['cat_id']) and $_POST['cat_id'] != "") {
																																																																							echo $_POST['cat_id'];
																																																																						} else { ?><?php } ?>',<?php echo $_REQUEST['type']; ?>,<?php echo $_REQUEST['p']; ?>,<?php echo $_REQUEST['sort']; ?>,'<?php echo $query_search; ?>','<?php echo $name; ?>','<?php if (isset($_POST['nearer']) and $_POST['nearer'] != "") {
																																																																																																																																							echo $_POST['nearer'];
																																																																																																																																						} else { ?><?php } ?>');" class="form-check-input filter_checkbox m-1">
													<div class="mylabel ml-4"><?php echo $name; ?></div>
												</li>
						...
						<div id="collapseFour" class="collapse show" aria-labelledby="headingFour" data-parent="#accordion">
							<div class="card-body">
								<?php
								$filter_query = mysqli_query($this->mysqlConfig(), "select DISTINCT pic_add_taluk from pic_addpost where  addpost_status = 1 and (pic_validity > $dates or pic_validity_auto=1) and pic_request=" . $_REQUEST['type'] . $filter_str . " and (`pic_title` LIKE  '%$query_search%' or `pic_tag` LIKE  '%$query_search%' or `pic_ads_id` LIKE  '%$query_search%' or `pic_admin_tag` LIKE  '%$query_search%') order by $order");
								...
								?>
									...
									<ul class="list-group">
										...
										?>
												<li class="list-group-item">
													<input <?php if (isset($_POST['nearer']) and $_POST['nearer'] == $name) { ?> checked <?php } else {
																																	} ?> id="nearer" name="nearer" value="<?php echo $name; ?>" type="radio" onchange="javascript:filterProducts_search(this,'<?php if (isset($_POST['cat_id']) and $_POST['cat_id'] != "") {
																																																																					echo $_POST['cat_id'];
																																																																				} else { ?><?php } ?>',<?php echo $_REQUEST['type']; ?>,<?php echo $_REQUEST['p']; ?>,<?php echo $_REQUEST['sort']; ?>,'<?php echo $query_search; ?>','<?php if (isset($_POST['location']) and $_POST['location'] != "") {
																																																																																																																															echo $_POST['location'];
																																																																																																																														} else { ?><?php } ?>','<?php echo $name; ?>');" class="form-check-input filter_checkbox m-1">
													<div class="mylabel ml-4"><?php echo $name; ?></div>
												</li>
	...
	public function filter()
	{
		...
		?>
			...
			$query_ads = mysqli_query($this->mysqlConfig(), "select * from pic_addpost where addpost_status = 1 and (pic_validity > $dates or pic_validity_auto=1) and (`pic_title` LIKE  '%$query_search%'  or `pic_discription` LIKE  '%$query_search%' or `pic_tag` LIKE  '%$query_search%' ) " . $str_location . "" . $str_nearer . "" . $str_cate . "  order by $order LIMIT 5 OFFSET " . $_REQUEST['offset'] . "");
			$count_rows = mysqli_num_rows($query_ads);
		...
		public function sorting()
		{
			...
			<div class="col-sm-12 col-md-12 col-lg-12">
				<h6 class="hint">
					Showing results for <span class="text-primary">"<?php echo $_REQUEST['search_pic']; ?>"</span>
				</h6>
			</div>
