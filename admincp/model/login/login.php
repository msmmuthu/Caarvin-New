...
<input type="hidden" name="post" value="login" />
    ...
    <?php
        ...
        $query_customer = mysqli_query($this->mysqlConfig(),"SELECT * FROM `pic_admin` where admin_username='$username' and admin_password='$password' limit 1");
        $query_customer_row = mysqli_fetch_array($query_customer);		
		//if($query_customer_row['staff_status']==1){
				
				require("config/session/session.php");
				$session = new session();
				$session -> create($query_customer_row['admin_id'],$query_customer_row['admin_username'],$query_customer_row['admin_id']);
			...
			//else{
			//echo '<span class="text-danger"><i class="fa fa-exclamation-circle fa-2x"></i></br>Invalid account details.</span></br><span class="hint">"Please check your email address & password."</span>';
			//}
    ?>
<p></p>
      <a class="btn btn-lg btn-primary btn-block" href="index.php"  style="background:#399bc8;">Go Back</a>
