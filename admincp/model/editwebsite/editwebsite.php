...
class editwebsite extends config{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' and isset($_FILES['photo'])){
		...
		?>
        	...
        	<div align="center"><a class="btn_sub_active" href="index.php?action=view&module=editwebsite&catid=<?php echo $categories_id; ?>&post=edit">Back</a></div>
        <?php
		}
		else{
		...
		if($name != ''){
				...
				$cat_query = mysqli_query($this->mysqlConfig(),"UPDATE `pic_website` SET `website_name` = '".$cat_name."',`website_url` = '".$cat_desc."',`logo`='".$name."' WHERE `pic_website`.`id` = $categories_id;");
		}else{
				$cat_query = mysqli_query($this->mysqlConfig(),"UPDATE `pic_website` SET `website_name` = '".$cat_name."',`website_url` = '".$cat_desc."' WHERE `pic_website`.`id` = $categories_id;");
		}
